...
public class LogInterceptor implements Interceptor {
            ...
            if (!hasRequestBody) {
                ...
                if (isPlaintext(buffer) && charset != null) {
                    ...
                    if (contentType != null && "json".equals(contentType.subtype())) {
                        logger.log(LOG_PREFIX + "\n" + GsonFormatter.format(bufferString));
                    }
                    logger.log(LOG_PREFIX + "--> END " + request.method()
    ...
    private static class GsonFormatter {

        private final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

        private final JsonParser PARSER = new JsonParser();

        private String _format(String source) {
            return GSON.toJson(PARSER.parse(source));
        }

        private static final GsonFormatter formatter = new GsonFormatter();

        private static String format(String source) {
            return formatter._format(source);
        }
    }
